...
- **Raw transcript is never persisted durably** *(amended by Patch 7.2 — see "Voice Intelligence working session" below: the transcript and analysis now also live in a TEMPORARY per-tab `sessionStorage` working session, never durable)*: as first built it lived only in React state while the screen was open — never SharePoint, never a Work Record, never Inbox Intelligence, never `localStorage`/`sessionStorage`/`IndexedDB`/cookies, never logged server-side (the API route logs only the caught error object, matching `app/api/inbox-intelligence/route.ts`'s existing discipline), never sent to analytics.
...
## Reviewed Contact Import (Patch 8F) — GREEN

A safe, review-first way to bring existing people/contact data into the durable Contact domain from Patch 8B. Governing principle, unchanged throughout: **imports provide evidence, durable Contacts remain human-reviewed truth.** No imported row may silently overwrite an existing Contact, and none may automatically become one — every creation still runs through the existing `ContactFormModal` / `ContactProvider.create()` path, one explicit human Save at a time. This is NOT bulk CRM ingestion.

- **Audit first** (per the patch's own preflight): no CSV/XLSX parsing infrastructure, file-upload UI, or import/migration code existed anywhere in the repo before this patch — confirmed by exhaustive search. `package.json` had no spreadsheet-parsing dependency. The only prior mention of "import" for Contacts was Patch 8B's own forward-reference reserving it for a later patch.
- **`lib/csv-parser.ts`** — a new, small, dependency-free, pure CSV parser (`parseCsv()`). Handles quoted fields, commas and embedded newlines inside quotes, doubled-quote (`""`) escaping, CRLF and LF line endings, a UTF-8 BOM, blank lines (skipped, not turned into a one-empty-cell row), and empty optional cells; throws a clear `CsvParseError` on an unterminated quote or a stray quote mid-field rather than silently misreading the row. No dependency added — XLSX is explicitly out of scope for this patch.
- **`lib/contact-import.ts`** — header-alias recognition (Name/Full Name required; Email/Email Address; Role/Title/Job Title; Organization/Organisation/District/Company — every other column ignored, never mapped into Notes), `resolveOrganizationByExactName()` (reuses `normalizeOrganizationName()` from `lib/organization-provider.ts`, exact match only, never fuzzy, never auto-creates an Organization), and `buildContactImportCandidates()` — a pure function that, per row, resolves the Organization and computes `ContactMatchCandidate[]` via **`lib/contact-matching.ts`'s `matchContactCandidates()`, reused completely unmodified** (no second duplicate-detection algorithm exists). The transient `ContactImportCandidate` type carries no `ProviderMetadata`, is never written to SharePoint, and is never assembled into any import-history record — `sourceFileName`/`sourceRowNumber` exist only so the human can cross-reference their own file.
- **`app/ContactImport.tsx`** — the review screen, reached via a new "Import Contacts" action on the Contacts screen (an internal view-state toggle, mirroring how Patch 8C's Contact Detail and this screen's own "Add Contact"/"Edit" flows already work — no new top-level `View`). File selection uses `FileReader` (not `File.prototype.text()`, which this repo's test/browser environment doesn't reliably support) and is entirely client-side. Each parsed row renders its own evidence (source row number, Name, Email, Role, original Organization text and its resolved name or "not resolved") above a reused `app/ContactMatchPanel.tsx` (Patch 8D) for the actual Match Existing / Add Person / Ignore decision — no second matching-UI component was built. **Match Existing never calls `updateContact()`** — it records only the chosen `contactAppId` in local review state. **Create New** opens the existing `ContactFormModal`, prefilled only with evidence present on that row (name/email/role/resolved organization — never Notes), and only an explicit human Save in that modal calls `ContactProvider.create()`. A session summary (total/unreviewed/matched existing/created/ignored) is transient component state, never persisted, never analytics. **No bulk action exists anywhere** — no Save All, Create All, Import All, or Accept All Strong Matches.
- **Same-session duplicate handling**: because `ContactImport` recomputes `buildContactImportCandidates()` from the app's live `contacts` prop (which already updates reactively after any `saveContact()` call — the same mechanism Patch 8D's Inbox/Voice "Add Person" already relies on), a Contact created for an earlier row in the same file becomes immediately matchable (as a `strong` email match, or `useful`/`review` by name) for a later duplicate row — without a separate session-contact cache. Every source row still remains independently visible and reviewable; rows are never silently deduplicated before review.
- **No SharePoint schema change of any kind**: no new list, no new column on `IU_Contacts`, no import-history list. The review session lives only in browser memory for the duration of the visit.
- **Zero AI calls** anywhere in the import path — file selection, parsing, header mapping, normalization, Organization resolution, and Contact matching are all fully deterministic.
- **No new Microsoft permissions** — scopes remain exactly `User.Read`, `Sites.ReadWrite.All`.
- **Test coverage**: `tests/csv-parser.test.ts` (quoted commas, escaped quotes, CRLF/LF, BOM, blank lines, empty optional cells, malformed-quote errors), `tests/contact-import.test.ts` (every header alias, missing-Name-column error, unknown-columns-ignored, Organization exact-resolution incl. null-on-no-match, the full Contact-matching ladder reused via `matchContactCandidates()`, multi-candidate retention, transient-provenance shape, same-session recomputation visibility), `tests/contact-import-ui.test.tsx` (entry point, file selection/parse-error display, candidate/evidence rendering, Match Existing incl. multi-candidate picking, Create New incl. exact prefill and unmapped-columns-never-in-Notes, a created Contact becoming matchable to a later duplicate row, Ignore, session-summary counts, no-bulk-action, zero network/AI calls, Match Existing never calling `ContactProvider.update()`, Contacts directory regression), `tests/contact-import-architecture-guardrails.test.ts` (Contact model unchanged incl. no `firstName`/`lastName`/`phone`/`organizationIds`/`projectIds`/durable-provenance field, no import-history model or file, no Organization auto-create, no fuzzy-matching library, no AI references, no new Microsoft permissions, no bulk-action strings, Match Existing/Create New source-level checks, no XLSX). Zero real SharePoint writes and zero real Anthropic calls in any automated test.
- **Manual verification**: a synthetic, non-tracked temporary CSV (outside the repo, in the session scratchpad, deleted before staging) was used to visually confirm the entry point, the Import Contacts screen's layout/copy, and back-navigation to an intact Contacts directory in the browser preview. **Actually selecting and uploading the file could not be exercised manually in this environment** — the available browser-automation tooling has no equivalent of Chrome DevTools Protocol's `DOM.setFileInputFiles` (confirmed: attempting to set the file input's value directly is rejected by the browser itself, `InvalidStateError`). The full file-select → parse → review → decide flow is instead thoroughly exercised by the 16 `contact-import-ui.test.tsx` tests, which use real `File`/`FileReader` objects via `@testing-library/user-event`'s `upload()` against the exact same component code.
- **Explicitly out of scope, not attempted**: XLSX support; a spreadsheet-parsing dependency; durable Contact provenance fields; an import-history record or list; bulk create/Save All/Accept All; a second Contact duplicate-detection algorithm; Organization auto-creation or fuzzy Organization matching; AI-assisted parsing or cleanup; new SharePoint lists/columns; new Microsoft permissions.
...
## Voice Intelligence working session (Patch 7.2) — GREEN

**ANALYZE ONCE. KEEP THE WORKING SESSION.** One long transcript can segment into many candidates the human processes one at a time; navigating around the app (or the post-save move to Today after a Work Record) used to unmount `VoiceIntelligence` and lose all of it, forcing a second paid Anthropic call. Voice Intelligence now keeps ONE temporary, per-tab working session.

- **Why it was lost**: `IUWorkTracker` renders `VoiceIntelligence` only while `view === "voice"`, and every piece of Voice state (`transcript`, `candidates`, `phase`, `usage`) was plain `useState` inside it — so any navigation, and the successful-save `setView("today")` after Log as work, unmounted it. (Cancelling the wizard alone never lost state — it is a modal over the still-mounted screen.)
- **Storage boundary — intentional and narrow**: browser `sessionStorage`, key `iu-work-tracker.voice-intelligence-session.v1` (`VOICE_SESSION_STORAGE_KEY`), schema version `1`. **Never** localStorage, SharePoint, the DataProvider, IndexedDB, cookies, the URL, Microsoft Graph, or Anthropic persistence. It survives navigation, unmount/remount, the Work Record handoff, and a same-tab refresh; **closing the tab clears it**. It is NOT durable institutional data, and the UI copy says exactly that ("Working session saved in this tab." / "Working session restored." + "Closing this tab clears the temporary Voice Intelligence session.") — never "saved", "backed up", or "cloud".
- **`lib/voice-intelligence-session.ts`** — a small, testable helper: `loadVoiceSession`/`saveVoiceSession`/`clearVoiceSession`/`withCandidateLogged`, all taking a Storage-like `{getItem,setItem,removeItem}` (so tests inject an in-memory fake). Reuses the existing `VoiceCandidate` type (no parallel Voice domain model): `VoiceReviewCandidate = VoiceCandidate & { id, selected, contactDecision?, loggedWorkRecordAppId? }`. The stored shape is `{ schemaVersion, transcript, phase, analyzed, candidates[], usage, savedAt }` — no tokens, keys, providers, or error text. Validation is Zod-based and deliberately LENIENT about candidate text (a title/detail the human cleared mid-edit restores as-is rather than failing the model's min-length rules).
- **What persists**: the raw transcript (also BEFORE analysis, with no fake candidates and never auto-analyzed), the analyzed candidates with their **stable ids** (ids are generated once at Analyze and restored, never regenerated on mount), every human edit (type/title/detail/duration removal), selection, removals (the remaining collection is what is stored), PERSON Contact-match decisions, the model/token usage line, which screen (`phase`) was showing, and the Logged marker. Writes are immediate on every change (no debounce — sessionStorage writes are cheap and a debounce could lose the last keystrokes on unmount).
- **Restore**: on mount, one synchronous `sessionStorage` read via a lazy `useState` initializer — **never an Anthropic call, never an automatic re-analysis**. Malformed JSON, a wrong `schemaVersion`, or a structurally invalid session is discarded (entry removed) and the normal empty screen shows; no raw parse error is ever surfaced.
- **Analyze stays the only AI trigger.** A failed Analyze leaves the stored transcript untouched; nothing retries or analyzes in the background.
- **Clear** (paste-screen "Clear" and review-screen "Analyze another transcript" now share one `startOver`) empties the transcript, every candidate and review decision, and **removes the storage key** — a refresh/return shows a clean screen. No new confirmation dialog was added (there was none before).
- **Storage failure never blocks anything**: quota/blocked/unavailable storage is caught everywhere; analysis, editing, and Log as work continue from in-memory state, and the status line is simply not shown (it is never claimed when a write failed). `VoiceIntelligence` takes an optional `storage` prop purely as a test seam; production passes nothing and uses `window.sessionStorage`.
- **Log as work — unchanged architecture**: the session is persisted first, then the EXISTING `openLog(draft, onSaved)` opens the EXISTING Universal Work Record wizard; no Work Record is created, no candidate is removed, nothing is cleared, no Anthropic call. No second form, no second save path.
- **Logged state — implemented, tied to a real success signal**: `IUWorkTracker.save()` already invokes the `onSaved` callback passed to `openLog` only after `createWorkRecord`/`updateWorkRecord` returns `success` (the same hook Inbox's "Create Work Record" uses). Voice passes a callback that marks the originating candidate (`loggedWorkRecordAppId`) — never on the click, a cancel, a validation failure, or a provider/conflict error. The badge ("Logged ✓") is Voice-session UI state only: **no Voice field was added to the Work Record model or SharePoint**, and no Voice source id is stored on a Work Record. The candidate stays visible and selected; "Log as work" becomes a de-emphasized "Log again" (not disabled — logging again stays possible on purpose).
- **Voice → Work Record, Contacts, Organizations, ACTION/KNOWLEDGE persistence: untouched.** PERSON/ORGANIZATION/DISTRICT candidates still create nothing; other candidate types still have no destination.
- **Zero SharePoint / Graph / Microsoft-permission / schema changes; zero new Anthropic behavior** (model, prompt, taxonomy, duration safeguard, dedupe all unchanged).
- **Known, deliberately unchanged**: a browser refresh returns the app to Home (the current view is not persisted) — the Voice session is restored when the human returns to Voice Intelligence. "Analyze another transcript" still discards the current session without a prompt, exactly as it always discarded the in-memory state.
- **Test coverage**: `tests/voice-intelligence-session.test.ts` (helper: round-trip of every preserved field, malformed/wrong-version/missing-structure rejection, clear, read/write/remove failure safety, module boundary), `tests/voice-intelligence-session-ui.test.tsx` (component + full-app: restore with zero fetches, edits/type/selection/removal persistence, malformed-session fallback, Clear, status copy, storage-failure safety, persist-before-handoff, Logged only after a real save and only on the originating candidate, cancel leaves the session recoverable, exactly one existing `createWorkRecord` call, no localStorage, no SharePoint imports), `tests/voice-session-test-utils.ts` (in-memory fake Storage — this environment's jsdom Storage is unreliable under Node 25). The three existing Voice test files gained a per-test `sessionStorage` key cleanup (jsdom shares storage within a file), and their two "touches no browser storage" assertions were rewritten to the new contract (no localStorage; only the Voice session key in sessionStorage). Zero real Anthropic calls in any test.

**Status: GREEN.**

---

## Voice Intelligence select + route (Patch 7.3) — GREEN

**AI EXTRACTS. HUMAN SELECTS. THE SYSTEM ROUTES.** The candidate checkboxes now mean something operational without turning every candidate into a Work Record.

- **Checkbox semantics**: `selected` means **"include this candidate in the current processing batch."** Unchecked candidates stay in the working session untouched and are simply not processed right now — unchecking never deletes anything. **Only "Remove" deletes a candidate.** (The summary line's third count was renamed from "ignored" to "not selected" to match; "Ignore" now only ever means a per-candidate routing decision.)
- **Selected tray** (`app/VoiceIntelligence.tsx`): appears only while ≥1 candidate is selected — "Process selected", "Select all" (only when something is unselected), and "Clear selection" (deselects only). Nothing is written by any of them.
- **Process selected panel** (`app/VoiceRoutingPanel.tsx`, an inline panel — not a modal): groups the SELECTED candidates by type with counts, "Ready destinations" first, then "No destination yet". Opening it writes nothing and makes no AI call. Its open/closed state is part of the Patch 7.2 session (`routingOpen`), so after the Log-as-work round trip (which ends on Today) the panel is still open on the next item.
- **Supported destinations — every one reuses an EXISTING path; nothing new is persisted**:
  - **Completed work → Work Records**: sequential, one item at a time ("Completed work 1 of 3" → **Log this work** / **Skip**), through the existing `openLog(draft, onSaved)` wizard and the single existing save path. Marked **Logged ✓** only when that save reports success (the 7.2 hook). No bulk write. Skip is transient (skipped items can be shown again) and loses nothing.
  - **Person → Contacts**: the existing `ContactMatchPanel` (Match Existing / Add Person / Ignore) and `ContactFormModal` — the same decision state the candidate card already used. A Contact created via Add Person also sets `contactCreated` → "Created Contact ✓".
  - **Organization / District → Organizations**: exact normalized-name matching (`lib/voice-routing.ts` — an ORGANIZATION candidate matches only non-district Organizations, a DISTRICT candidate only `type === "district"`, mirroring `resolveEmailAnalysisEntities`; every exact match is shown so ambiguity reaches the human), **Match Existing** / **Create Organization** / **Ignore**. Create opens the existing `OrganizationFormModal` (District prefills `type: "district"` — there is still no separate District model) and only an explicit Save writes, once, through the existing `OrganizationProvider`.
  - **Project → Projects**: exact normalized-name matching only (never fuzzy), **Match Existing** / **Create Project** / **Ignore**; Create opens the existing `ProjectFormModal` (name + detail prefilled) and only an explicit Save writes, through the existing `ProjectProvider`.
- **No durable destination exists yet for**: **Action, Knowledge, Idea, Decision, Question.** Discovery confirmed there is no Action/Knowledge/Idea/Decision/Question domain, provider, or list — Action Center is only a derived view over Inbox Intelligence records (its action items belong to email-derived records, so forcing Voice actions in there would be a semantic lie). The panel shows each as "<Type> — destination not yet implemented", keeps the candidates selected and intact, offers **Copy** (plain text to the clipboard — writes to no store), and nothing is fake-persisted. No new SharePoint list was created to satisfy routing.
- **Processed state** is session-only and set **only after a successful durable action**: "Logged ✓", "Created Contact ✓", "Matched Organization ✓" / "Created Organization ✓", "Matched Project ✓" / "Created Project ✓" (`processedChipLabel`). Decisions are additive optional fields on the Patch 7.2 session (`routing`, `contactCreated`, `routingOpen`; a 7.2 session without them still restores; schema version stays 1) and never touch Work Record, Contact, Organization, or Project models. A `matched`/`created` decision also stores the entity's name purely as a display fallback (e.g. an in-memory Preview record after a refresh).
- **Extraction (not duplication)**: `ProjectFormModal` and `OrganizationFormModal` (with `emptyProjectDraft` / `emptyOrganizationDraft` / `ORGANIZATION_TYPE_LABELS`) moved out of `app/IUWorkTracker.tsx` into `app/ProjectFormModal.tsx` / `app/OrganizationFormModal.tsx`, exactly as Patch 8D extracted `ContactFormModal` — one form, one provider each, shared by the Projects/Organizations screens and Voice. Only change: `onSaved` now also receives the saved entity (existing callers ignore it). `VoiceIntelligence` receives `saveProject/updateProject/saveOrganization/updateOrganization` from `IUWorkTracker` and never calls them itself — only the reused form's explicit Save does.
- **Guardrails**: no bulk writes, no auto-created Contact/Organization/Project, no AI (reclassification, matching, ranking — all deterministic), no localStorage (Patch 7.2's sessionStorage-only session is unchanged; Clear / "Analyze another transcript" still destroys it, routing state included), no SharePoint schema, no Microsoft permission change, no Anthropic prompt/model change. The existing `tests/organization-architecture-guardrails.test.ts` rule was refined from "Voice never references saveOrganization" to "Voice never CALLS it — it only passes the provider functions to the reused form" (Inbox is still fully forbidden).
- **Test coverage**: `tests/voice-routing.test.ts` (destinations, grouping, exact matching incl. ambiguity and no-fuzzy, processed chips, additive/backward-compatible session schema), `tests/voice-intelligence-routing-ui.test.tsx` (tray semantics, grouping/mixed selection, honest unsupported destinations + Copy, sequential Completed-work flow incl. cancel/skip/real-wizard full-app run, Person/Organization/Project match + create through the real Memory providers behind spies, persistence/remount/Clear, zero-AI, no localStorage, static no-duplicate-provider checks). Zero real Anthropic calls; zero SharePoint.
- **Recommended next durable destination**: a generic **Action/Task** domain (the largest stranded type), followed by a lightweight Knowledge/Idea/Decision/Question capture — each needs its own explicit design and provisioning decision first.
