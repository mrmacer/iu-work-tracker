...
- **Meeting content is fully transient**: `MeetingDraft` (title, date, meetingType, attendeesText, agendaText, notesText — `lib/meeting-minutes.ts`) lives only in `app/MeetingNotes.tsx`'s React state. No SharePoint list, no `localStorage`/`sessionStorage`/`IndexedDB`/cookies, no URL state. The screen says so plainly: "Meeting Notes V1 is a review workspace. Meeting content is not saved yet."
...
- No SharePoint schema, no new Microsoft permissions, and no calendar/Outlook/Teams integration were added — this is intentionally manual V1.

**Recommended next step (not started)**: **Patch 6B — Meeting Persistence** — a SharePoint schema/durability design for Meeting Notes, only after the V1 workflow above is visually and functionally approved. Do not begin this without separate explicit instruction.
