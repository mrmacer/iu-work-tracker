...
- **CURRENT FALLBACK HOST vs NEXT DEPLOYMENT TARGET**: `https://iu-work-tracker.gmacer.chatgpt.site/` (ChatGPT/OpenAI Sites, commit `80eee7f`) remains the live fallback host, untouched by this local branch. This branch targets **Vercel** as the next deployment target — it has not yet been deployed anywhere. Do not confuse "the hosted Sites app still runs the pre-Patch-3A vinext/Cloudflare build" with "this repository still runs on that architecture" — they now differ until a Vercel deployment happens and is verified.
