...
**Status: awaiting the SharePoint schema approval gate.** No `IU Meeting Records` list has been created or modified, and `NEXT_PUBLIC_SHAREPOINT_IU_MEETING_RECORDS_LIST_ID` remains unset — `selectMeetingRecordProvider()` resolves to the memory provider in every environment today.
